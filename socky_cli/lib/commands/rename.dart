...
class RenameCommand extends Command {
  ...
  run() async {
    ...
    if (choice) {
      ...
      var sockyYamlFile =
          new File.fromUri(Directory.current.uri.resolve('socky.yaml'));

      if (!await sockyYamlFile.exists()) {
        throw new Exception('No socky.yaml found in current directory.');
      } else {
        sockyYamlFile.writeAsStringSync('''name: $newName
''');
        await renameDartFiles(Directory.current, 'socky_project', newName);
